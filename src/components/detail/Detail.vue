    ...
    <div class="card"
      ...
      <section class="subject-info">
        <div class="right">
          <a href="javascript:;">
            <img width="90"
              :src="subject.images.medium | handleImageUrl"
              :alt="subject.title"
              class="cover">
          </a>
        </div>
        <div class="left">
          <p class="rating">
            ...
            <span v-if="collected === true">
              <i class="collect-on"
                @click="collectMovie"></i>已收藏</span>
            <span v-else>
              <i class="collect"
                @click="collectMovie"></i>收藏</span>
          </p>
          <p class="meta">
      ...
      <section class="subject-genres"
        v-if="subject.genres.length">
        <h2>电影类型</h2>
        <p>
          <span v-for="item in subject.genres"
            :key="item">{{item}}</span>
        </p>
      </section>
      <section class="subject-casts"
        v-if="subject.directors.length">
        <h2>导演</h2>
        <div class="casts-wrap">
          <div class="casts-item"
            v-for="item in subject.directors"
            :key="item.id">
            <router-link :to="{ name: 'Celebrity', params: { id: item.id }}">
              <img v-if="item.avatars"
                :src="item.avatars.medium | handleImageUrl"
                width="90"
                height="120">
              <p class="name">姓名：{{item.name}}</p>
              <p class="intro">
                <a :href="item.alt">豆瓣主页</a>
              </p>
            </router-link>
          </div>
        </div>
      ...
      <section class="subject-casts"
        v-if="subject.casts.length">
        <h2>主演列表</h2>
        <div class="casts-wrap">
          <div class="casts-item"
            v-for="item in subject.casts"
            :key="item.id">
            <router-link :to="{ name: 'Celebrity', params: { id: item.id }}">
              <img v-if="item.avatars"
                :src="item.avatars.medium | handleImageUrl"
...
export default {
  ...
  updated() {
    ...
    const { average } = this.subject.rating;
